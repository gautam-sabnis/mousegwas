...
source("/projects/kumar-lab/sabnig/Pipelines/gwas/R/get_genes.R")
source("/projects/kumar-lab/sabnig/Pipelines/gwas/R/prepare_inrich_input.R")
source("/projects/kumar-lab/sabnig/Pipelines/gwas/R/plot_gemma_lmm.R")
...
#args$outdir <- '/projects/compsci/jgeorge/USERS/chouda/JABS/gwas_grooming_reproducibility/mousegwas/work/2e/16ce1bb4be559c1be84556402641eb/outdir'  # added
#args$outdir <- '/projects/kumar-lab/sabnig/Pipelines/gwas/work/1d/040ce0356e3e182edc378b529ef123/outdir'
#args$yaml <- 'jabs_ac.yaml'  # added
#args$plotdir <- 'plots'  # added
...
for (i in 1:length(phenos)) {
  ...
  print("Reached: after_plot_gemma_lmm:1")
  pname <- phenos[i]
  lilp[[pname]] <- pp
  allsnps <- pp$gwas$rs
  if (is.null(pvalmat)) {
  ...
  print("Reached: after_plot_gemma_lmm:2")
  ...
  print("Reached: after_plot_gemma_lmm:3")
}
...
for (i in 1:length(lilp)) {
  ...
  message(print("ok:1"))
  affgen <-
    get_genes_ac(expp[expp$ispeak == T, ], dist = 1000, annot = annot)
  message(print("ok:2"))
  if (nrow(affgen) > 0) {
